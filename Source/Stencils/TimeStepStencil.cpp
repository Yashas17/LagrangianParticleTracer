...
void Stencils::TimeStepStencil::apply(FlowField& flowField, int i, int j) {
  ...
  const RealType* const vt = flowField.getVt().getScalar(i,j);
...
//   TODO: confirm that Re should be replaced by vTotal
  ...
  if (velocity[0] != 0 && velocity[1] != 0) {
    cell_dt = std::min(
      1 / ( vTotal * factor ),
      std::min(
        parameters_.timestep.dt, std::min(dx / velocity[0], dy / velocity[1])
      )
    );
  } else {
...
void Stencils::TimeStepStencil::apply(FlowField& flowField, int i, int j, int k) {
  const RealType* const vt = flowField.getVt().getScalar(i,j,k);
